...
def policy_gradient(mu, max_iters, gamma, eta, T, samples):
    ...
    initial_policy = {(s,i): [1/M]*M for s in range(S) for i in range(N)}
    ...
    for t in range(max_iters):
        ...
        for agent in range(N):
            for st in range(S):
                ...
                if np.random.uniform() < reinit_prob:
                    policy[st, agent] = np.array(copy.deepcopy(initial_policy[st, agent]))
                else:
...
full_experiment(4,500,0.0001,20,10)
